...
<template>
  <header>
    <nav class="fixed top-0 left-0 z-50 w-14 h-14">
      <button aria-controls="nav-and-sidebar"
        ...
        data-drawer-target="nav-and-sidebar" data-drawer-toggle="nav-and-sidebar" type="button">
        <span class="sr-only">Open sidebar</span>
        <Bars3Icon aria-hidden="true" class="h-6 w-6 text-gray-500 dark:text-gray-400" />
      </button>
    </nav>

    <aside id="nav-and-sidebar" aria-label="Sidebar"
      ...
      <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
        ...
        <div class="flex justify-end items-center sm:w-64 absolute right-0 px-3 py-3">
          <div class="flex-auto me-3">
            <div class="flex ml-2">
              <img alt="haenno profile picture" class="h-11 mr-3 rounded-full" src="./assets/profilepic.jpg" />
              <span
                class="self-center italic text-xl font-semibold sm:text-2xl whitespace-nowrap text-gray-700 dark:text-white">haenno</span>
            </div>
          </div>
        ...
        <div class="absolute top-20 w-56">
          <ul class="space-y-2 font-medium">
            ...
            <NavLinkStyle :navLink="{ text: 'About', target: 'about' }">
              <InformationCircleIcon aria-hidden="true"
                class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
            </NavLinkStyle>

            <NavLinkStyle v-if="!userAuthStore.userState" :navLink="{ text: 'Login', target: 'login' }">
              <ArrowRightOnRectangleIcon aria-hidden="true"
                class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
            </NavLinkStyle>

            <NavLinkStyle v-if="userAuthStore.userState" :navLink="{ text: 'Tasks', target: 'tasks' }">
              <RectangleStackIcon aria-hidden="true"
                class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
              <template v-slot:hint>
                <span v-if="userAuthStore.userState"
            ...
            <NavLinkStyle v-if="userAuthStore.userState" :navLink="{ text: 'Inbox', target: 'inbox' }">
              <InboxArrowDownIcon aria-hidden="true"
                class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
              <template v-slot:hint>
                <span
            ...
            <NavLinkStyle v-if="userAuthStore.userState" :navLink="{ text: 'Logout', target: '/#' }"
              @click="userAuthStore.logout">
              <ArrowLeftOnRectangleIcon aria-hidden="true"
                class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
            </NavLinkStyle>

            <NavLinkStyle v-if="!userAuthStore.userState" :navLink="{ text: 'Register', target: 'signup' }">
            ...
            <li>
              <span
                ...
                active-class="dark:bg-gray-700 bg-gray-200" data-drawer-hide="nav-and-sidebar"
                data-drawer-target="nav-and-sidebar" href="#">
                <QuestionMarkCircleIcon aria-hidden="true"
                  class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
                <span class="flex-1 ml-3 whitespace-nowrap">Logged in? {{ userAuthStore.userState }}</span>
              </span>
            </li>
          </ul>
        </div>
