      ...
      <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
        ...
        <div class="absolute top-20 w-56">
          <ul class="space-y-2 font-medium">
            ...
            <NavLinkStyle v-if="!userAuthStore.userStateIsLoggedIn" :navLink="{ text: 'Login', target: 'login' }">
              <ArrowRightOnRectangleIcon aria-hidden="true"
                class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
            </NavLinkStyle>

            <NavLinkStyle v-if="userAuthStore.userStateIsLoggedIn" :navLink="{ text: 'Tasks', target: 'tasks' }">
              <RectangleStackIcon aria-hidden="true"
                class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
              <template v-slot:hint>
                <span v-if="userAuthStore.userStateIsLoggedIn"
            ...
            <NavLinkStyle v-if="userAuthStore.userStateIsLoggedIn" :navLink="{ text: 'Inbox', target: 'inbox' }">
            ...
            <NavLinkStyle v-if="userAuthStore.userStateIsLoggedIn" :navLink="{ text: 'Logout', target: 'logout' }">
              <ArrowLeftOnRectangleIcon aria-hidden="true"
                class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" />
            </NavLinkStyle>

            <NavLinkStyle v-if="!userAuthStore.userStateIsLoggedIn" :navLink="{ text: 'Register', target: 'signup' }">
